...
public class Act_1_hoverArea : MonoBehaviour
{
    ...
    public Act_1_gameController A1_gameControllerScript; // A1_gameControllerスクリプトの参照を格納するため
    ...
    void Start()
    {
        ...
        {
            A1_gameControllerScript = FindObjectOfType<Act_1_gameController>(); //  Act_1_gameControllerスクリプトをシーン内から探して参照を取得
        }
    }
    ...
    void OnMouseOver()
    {
        ...
        if (Input.GetMouseButtonDown(0)) // 左クリックが押された時
        {
            if (currentLine == null)
            {
                ...
                A1_gameControllerScript.audioSourceA1.PlayOneShot(A1_gameControllerScript.leftClickClip);


                if (pointA.name == "circle1")
                {
                    A1_gameControllerScript.isHorizontal_1_LineCreated = true;
                    Debug.Log("A1_GameControllerScript.isHorizontal_1_LineCreated: " + A1_gameControllerScript.isHorizontal_1_LineCreated);
                }
                else if (pointA.name == "circle3")
                {
                    A1_gameControllerScript.isHorizontal_2_LineCreated = true;
                    Debug.Log("A1_GameControllerScript.isHorizontal_2_LineCreated: " + A1_gameControllerScript.isHorizontal_2_LineCreated);
                }
        ...
        else if (Input.GetMouseButtonDown(1)) // 右クリックが押された時
        {
            if (currentLine != null)
            {
                ...
                Destroy(currentLine);
                currentLine = null;
                A1_gameControllerScript.audioSourceA1.PlayOneShot(A1_gameControllerScript.rightClickClip);

                Debug.Log("Horizontal line destroyed");

                if (pointA.name == "circle1")
                {
                    A1_gameControllerScript.isHorizontal_1_LineCreated = false;
                    Debug.Log("A1_GameControllerScript.isHorizontal_1_LineCreated: " + A1_gameControllerScript.isHorizontal_1_LineCreated);
                }
                else if (pointA.name == "circle3")
                {
                    A1_gameControllerScript.isHorizontal_2_LineCreated = false;
                    Debug.Log("A1_GameControllerScript.isHorizontal_2_LineCreated: " + A1_gameControllerScript.isHorizontal_2_LineCreated);
                }
            }
        }
    ...
    void OnDrawGizmos()
    {
        if (pointA != null && pointB != null)
        {
            ...
            Gizmos.DrawWireCube(midPoint, new Vector3(Vector3.Distance(pointA.transform.position, pointB.transform.position), A1_gameControllerScript.hoverAreaWidth, 0.1f)); // 中間地点に四角形を描画
        }
    }
}
