...
public class characterInfoHoverT0 : MonoBehaviour
{
    ...
    public Dictionary<string, string[]> defaultCharacterInfoDict = new Dictionary<string, string[]>
    {
        ...
        { "king", new string[] { "国王", "○ 庭師の少年を信頼している。", "○ 長生薬の秘方が欲しい。", "○ 財宝を奪われたら、メカニックを雇って宝庫を強化する必要がある。" } }
    };
    ...
    void Start()
    {
        ...
        // フォントの設定
        dotFont = gameControllerScript.dotFont;
    }
    ...
    void OnMouseExit()
    {
        ...
        if (charaInfoInstance != null)
        {
            ...
            {
                gameControllerScript.charaKnightInfoChecked = true;
                Debug.Log("gameControllerScript.charaKnightInfoChecked: " + gameControllerScript.charaKnightInfoChecked);
            }
            else if (character.name == "hunter")
            {
                gameControllerScript.charaHunterInfoChecked = true;
                Debug.Log("gameControllerScript.charaHunterInfoChecked: " + gameControllerScript.charaHunterInfoChecked);
            }
        }
    }
