...
public class Act_1_4_hoverArea : MonoBehaviour
{
    ...
    public Act_1_4_gameController Act_1_4_gameControllerScript;

    private Dictionary<int, int[]> horizontalLines = new Dictionary<int, int[]>(); // 横線の情報を保存するディクショナリ
    // Start is called before the first frame update
    void Start()
    {
        if (Act_1_4_gameControllerScript == null)
        {
            Act_1_4_gameControllerScript = FindObjectOfType<Act_1_4_gameController>(); //  Act_1_3_gameControllerスクリプトをシーン内から探して参照を取得
        }
    }
    ...
    void OnMouseOver()
    {
        ...
        if (Input.GetMouseButtonDown(0)) // 左クリックが押された時
        {
            if (currentLine == null)
            {
                ...
                Act_1_4_gameControllerScript.audioSourceA1_4.PlayOneShot(Act_1_4_gameControllerScript.leftClickClip);


                if (pointA.name == "circle1")
                {
                    Act_1_4_gameControllerScript.isHorizontal_1_LineCreated = true;
                    Debug.Log("Act_1_4_gameControllerScript.isHorizontal_1_LineCreated: " + Act_1_4_gameControllerScript.isHorizontal_1_LineCreated);
                }
                else if (pointA.name == "circle3")
                {
                    Act_1_4_gameControllerScript.isHorizontal_2_LineCreated = true;
                    Debug.Log("Act_1_4_gameControllerScript.isHorizontal_2_LineCreated: " + Act_1_4_gameControllerScript.isHorizontal_2_LineCreated);
                }
                else if (pointA.name == "circle5")
                {
                    Act_1_4_gameControllerScript.isHorizontal_3_LineCreated = true;
                    Debug.Log("Act_1_4_gameControllerScript.isHorizontal_3_LineCreated: " + Act_1_4_gameControllerScript.isHorizontal_3_LineCreated);
                }
            }
        }
        else if (Input.GetMouseButtonDown(1)) // 右クリックが押された時
        {
            if (currentLine != null)
            {
                ...
                Destroy(currentLine);
                currentLine = null;
                Act_1_4_gameControllerScript.audioSourceA1_4.PlayOneShot(Act_1_4_gameControllerScript.rightClickClip);

                Debug.Log("Horizontal line destroyed");

                if (pointA.name == "circle1")
                {
                    Act_1_4_gameControllerScript.isHorizontal_1_LineCreated = false;
                    Debug.Log("A1_GameControllerScript.isHorizontal_1_LineCreated: " + Act_1_4_gameControllerScript.isHorizontal_1_LineCreated);
                }
                else if (pointA.name == "circle3")
                {
                    Act_1_4_gameControllerScript.isHorizontal_2_LineCreated = false;
                    Debug.Log("A1_GameControllerScript.isHorizontal_2_LineCreated: " + Act_1_4_gameControllerScript.isHorizontal_2_LineCreated);
                }
                else if (pointA.name == "circle5")
                {
                    Act_1_4_gameControllerScript.isHorizontal_3_LineCreated = false;
                    Debug.Log("Act_1_4_gameControllerScript.isHorizontal_3_LineCreated: " + Act_1_4_gameControllerScript.isHorizontal_3_LineCreated);
                }
            }
        }
    ...
    void OnDrawGizmos()
    {
        if (pointA != null && pointB != null)
        {
            ...
            Gizmos.DrawWireCube(midPoint, new Vector3(Vector3.Distance(pointA.transform.position, pointB.transform.position), Act_1_4_gameControllerScript.hoverAreaWidth, 0.1f)); // 中間地点に四角形を描画
        }
    }
}
